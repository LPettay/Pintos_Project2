...
process_execute (const char *file_name) 
{
  ...
  /* Lance Pettay 3/1/17 - Tokenize file_name such that arguments can be passed. 

     Update: 3/2/17 - This won't work since 'argv[]' is needed outside of this
     function and to make it global would be inefficient. I have a better solution.
  int argc = 0;
  char *argv[10];
  char *token, *save_ptr;
...
setup_stack (void **esp, const char *file_name, char **save_ptr) 
{
  ...
  /* Lance Pettay - 3/2/17 - The following sections set up arguments on the
     stack. */
  int argc = 0;
  char *argv[30];
  char *token;

  /* LP - Determines addresses and pushes arguments + null pointer sentinel onto stack. */ 
  ...
  /* LP - Align stack for word-aligned access. */
  *esp -= (size_t) *esp % 4;

  /* LP - Push address of each string on to the stack (right to left) */
  int i;
  argv[argc] = 0;
  for(i = argc; i >= 0; i--)
  {
  ...
  /* LP - Push argv, argc, and fake return address */
  char *temp = *esp;
  *esp -= sizeof(char *);
